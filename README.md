...
| `skills/wave/SKILL.md` | `$wave <slug-or-goal>` — start or enter a multi-agent wave |
| `skills/gate/SKILL.md` | `$gate --pr <number>` — merge-gate check for a multi-agent task |
| `skills/digest/SKILL.md` | `$digest <slug>` — wave progress summary |
...
- **几个会话命令**：`$checkpoint`（中途存档）、`$handoff`（结束会话并更新状态）、`$team-progress`（看队友近期进展）、`$docs-refresh`（把过时文档刷新到最新）。多 agent 场景还有 `$wave`（起/进入 wave）、`$gate --pr <n>`（合并前检查）、`$digest <slug>`（wave 进展摘要）。
